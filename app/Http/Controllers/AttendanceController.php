    ...
    public function checkHalfDay(Request $request)
    {
        if($request->type == 'bulkMark')
        {
            ...
            if ($request->has('user_id') && $request->user_id !== null) {
                $leaves = Leave::whereIn('user_id', $request->user_id)
                    ...
                    ->whereIn('duration', ['half day', 'single'])
                    ->get();
            }

            if(isset($leaves) && $leaves->isNotEmpty())
            {
                $this->halfDayExist = $leaves->duration == 'half day' ? true : false;
                $this->requestedHalfDay = $request->half_day;

                $fullDay = $leaves->duration == 'single' ? 'no' : 'yes';
                $this->fullDayExist = $leaves->duration == 'single' ? true : false;
        ...
        else
        {
            $leave = Leave::where([
                ['user_id', $request->user_id],
                ['leave_date', $request->attendance_date]
                ])->whereIn('duration', ['half day', 'single'])->first();
            ...
            if(isset($leave))
            {
                ...
                $fullDay = $leave->duration == 'single' ? 'no' : 'yes';
                $this->fullDayExist = $leave->duration == 'single' ? true : false;
                $this->requestedFullDay = $fullDay;
